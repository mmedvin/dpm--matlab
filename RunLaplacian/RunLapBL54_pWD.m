...
for	   LinearSolverType = 0
	...
	for n=1:6 %6 %run different grids
        ...
        elseif isequal(DiffOp , @Tools.DifferentialOps.LaplacianOpBCinMat) || isequal(DiffOp , @Tools.DifferentialOps.LaplacianOpBCinMat4OrdrConst)
            DiffOpParams = struct(   'BC_y1',  (Grid.x(1).^ExParams.c)       .*(Grid.y.^ExParams.d).', ...
                                        'BC_yn',  (Grid.x(end).^ExParams.c)     .*(Grid.y.^ExParams.d).',...
                                        'BC_x1',( (Grid.x.^ExParams.c) .*(Grid.y(1).^ExParams.d)       ), ...
		...
		ExtPrb =  Solvers.ExteriorLaplacianSolver(struct( ...
                  ...
                  'SourceHandle'      , @Tools.Source.LaplaceSource_BL54_Exterior, ...
