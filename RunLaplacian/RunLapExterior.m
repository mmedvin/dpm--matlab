...
%ExParams.r  = ExParams.r0;
	...
	for n=1:5 %run different grids
		...
		xi(ExtPrb.GridGamma) = ...
			ExtPrb.W0(ExtPrb.GridGamma,:)*Basis.cn0 + ExtPrb.W1(ExtPrb.GridGamma,:)*cn1 + ExtPrb.Wf.W(ExtPrb.GridGamma);
...
% 		xi(ExtPrb.GridGamma) = xiex; %test
    ...
    %%%%%%%%%%%%%%
		...
		fprintf('b=%-5.2d,C=%-5.2d,NBss0=%d,NBss1=%d,N=%-10dx%-10d\t ebinf=%d\tetinf=%d\ttimeA=%d\ttimeE=%d\n',ExParams.B,ExParams.C,Basis.NBss0,Basis.NBss1, Nx,Ny,full(ebinf(n)),full(etinf(n)),t1,t2-t1);
