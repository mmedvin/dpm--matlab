function NavierStocksHalfEllipeseBAD
%this version supposed to work with gamma that sits on boundary,
% however it never worked
	...
	f   =@(phi) Omega(FocalDistance,Eta0,phi,ExParams);
	fn   =@(phi) Psi(FocalDistance,Eta0,phi,ExParams);
...
for	   LinearSolverType = 0
	...
	for n=1:4 %6 %run different grids
        ...
        OmegaSetup  = struct( 'Basis'     , Basis, ...
                'Grid'              , Grid, ...
                'CoeffsHandle'      , @Tools.Coeffs.ConstLapCoeffs, ...
                'CoeffsParams'      , struct('a',1,'b',1,'sigma',-k^2), ...
                ...
                'SourceHandle'      , @Tools.Source.NavierStokesTestSource, ...
		...
		OPrb =  Solvers.InteriorLaplacianSolver(OmegaSetup);
			
        PsiSetup = OmegaSetup;
        PsiSetup.CoeffsParams = struct('a',1,'b',1,'sigma',0);
        PPrb =  Solvers.ExteriorHomoLaplacianSolver(PsiSetup);
		%------------------------------------------------------------------
	if 1
...
function p = Psi(FocalDist,eta,phi,ExParams)
	x = FocalDist*cosh(eta).*cos(phi);
	y = FocalDist*sinh(eta).*sin(phi);
	
	p = exp(ExParams.c*x).*sin(ExParams.d*y);
end

function o = Omega(FocalDist,eta,phi,ExParams)
    o = 3*Psi(FocalDist,eta,phi,ExParams);
end
