...
function mi__tryPointInCircle
    ...
    src1 = mi__SARUtils.createPointWithAmplitude(2.6, rCenter + [0.3; 0.7]); 
    src2 = mi__SARUtils.createPointWithAmplitude(2.3, rCenter + [0; 0]); 
...
function testFFPfromPointInside(mesh, k, phi, pointPos)
    ...
    src_inside = mi__SARUtils.createPointWithAmplitude(2.3, pointPos); 
...
function testFFPfromPointOutside(mesh, k, phi, pointPos)
    ...
    src_outside = mi__SARUtils.createPointWithAmplitude(2.3, pointPos); 
...
function traj = doFFP_circle(k, phi, mesh, field)
    ...
    traj = mi__SARUtils.doFFP_curve(k, phi, curve, field); 
end
...
function plotTraj(traj)
    ...
    subplot(121); plot(traj.phi_refl, abs(traj.ffp), 'b-.'); grid on; title('abs')
    subplot(122); plot(traj.phi_refl, angle(traj.ffp), 'b-.'); grid on; title('phase')    
...
function field = getFieldFromPoint(pointSrc, mesh, k)
    ...
    besselKind = mi__SARUtils.besselKind; 
