...
[u1,ExtPrb1] = Solve(BC,Setup1);
[u2,ExtPrb2] = Solve(BC,Setup2Shited);
...
figure
plot(tt,real(m3),'r+',tt,real(m2),'bo')
legend('u3=u1_shifted','u2shifted')
figure, 
plot(tt,imag(m3),'r+',tt,imag(m2),'bo')
legend('u3=u1_shifted','u2shifted')
...
u_tot2 = u2 + Uinc(UincParams2,Th2,IncAng,k);
%u_tot3 = u3 + Uinc(UincParams2,Th2,IncAng,k);
u_tot3 = u3 + Uinc(UincParams1,Th1,IncAng,k);

figure
plot(ExtPrb1.Scatterer.th,abs(u_tot3(ExtPrb1.GridGamma)),'r+',ExtPrb2.Scatterer.th,abs(u_tot2(ExtPrb2.GridGamma)),'bo')
...
%subplot(133), DrawField(u_tot3,ExtPrb2,Setup2Shited,ExParams2,IncAng,'Tot field3(f1 shifted)',fig);
...
[FFP_plus2,FFP_minus2] = getFFP(u2,Setup2Shited,ExParams2);
...
arg = (xhat(1)-KincHat(1))*Shift(1) + (xhat(2)-KincHat(2))*Shift(2); Title = 'FFP minus, arg';
%     arg =- arg; title('FFP plus, -arg'), Title = 'FFP plus, -arg'; warning('wrong sign!')


F0 = FFP_minus1.ffp;
ShitftedF0 = F0.*exp(1i*k*arg); %approx using
F1 = FFP_minus2.ffp; % computed from the field
