...
function traj = tryOneLayer(k, phi_xhat, R, rCenter, theta, reflectivity_const, phi)
    ...
    [scattField, q_inc_z] = mi__SARUtils.getScattField(k, phi_xhat, reflector, isCheckShallow); 
    
    currentFigure = gcf;     
    figure('units', 'normalized', 'position', [0.02 0.1, 0.9, 0.4], 'name', 'one layer'); 
...
function traj = tryTwoLayers(k, phi_xhat, R, rCenter, theta, reflectivity_const, phi)
    ...
    currentFigure = gcf;  
    figure('units', 'normalized', 'position', [0.02 0.1, 0.9, 0.4], 'name', 'two layers'); 
